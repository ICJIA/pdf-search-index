...
**Seven adversarial red/blue team audit passes** to date. **Zero unaddressed exploitable issues** at the current release (v1.4.0).

- **2026-05-16 (v1.0.1 → v1.0.2 audit, full pass).** 21 findings: 5 Critical / 8 Important / 8 Minor. Shipped fixes: 4 Critical + 5 Important + 2 Minor. Deferred: C2 SSRF allowlist (v1.1), I2 cache-key URL normalization (v2.0), I5 CLI sitemap hardening (v1.1), I6 `maxUrls` cap (v1.1), the remaining Minor items.
...
- **2026-05-16 (v1.0.5 verification pass).** Re-verified all 11 prior 1.0.2 fixes still in place at v1.0.5 source. **0 new Critical/Important/Minor findings.** 1 Informational (V8: `snippetHTMLFor` `separator` parameter is concatenated raw — documented via JSDoc note).
- **2026-05-17 (v1.1.0 multi-format audit).** Audited the new DOCX/PPTX/XLSX surface, `officeparser` integration, format-mismatch detection, Office-aware error categorization. All 11 prior fixes verified. **0 new Critical/Important/Minor findings.** 1 Informational (V9: officeparser dynamic-import — same risk class as existing `unpdf`/`fuse.js` dynamic imports).
- **2026-05-17 (v1.2.0 perf/security-extension audit).** Audited the new `maxUrls` cap (closes I6), inflate-bomb defense (closes v1.1 deferral), prebuilt Fuse index, `/worker` re-export. All 11 prior fixes verified. **0 new Critical/Important findings.** 2 Minor (F1 maxUrls normalization, F2 prebuild HTML-safe encoding) + 2 Informational (F3 worker error UX, F4 `.d.ts` `any[]` arity) — F1/F2/F3 fixed before v1.2.0 publish; F4 documented as non-actionable.
- **2026-05-17 (v1.2.1, no new audit).** Runtime byte-identical to 1.2.0. Tightened `officeparser` peer-dep pin from `^5.0.0` to exactly `5.2.2` (the audited version) as supply-chain hardening. Documented v2.0 escape hatches if officeparser becomes unmaintained: fork to `@icjia/office-parser`, or replace with `mammoth` + `xlsx` + custom PPTX. **Vendoring** (full npm-takedown protection) tracked for v1.3.0.
- **2026-05-17 (v1.3.0 search-engine-entries audit).** Audited the two new adapter entries (`/flexsearch` and `/pagefind`), the officeparser promotion to direct dep, the new tsup entries. **0 Critical / 0 Important.** 3 Minor (V13-1 symlink jail, V13-2 baseUrl escape, V13-3 format-injection) — all fixed before v1.3.0 publish. 2 Informational (V13-4 pagefind escapeHTMLText control-byte scrub, V13-5 dynamic-import error message scrub) — deferred to v1.4.
- **2026-05-17 (v1.4.0 supply-chain-hardening audit).** Audited the vendored officeparser source (`packages/core/src/vendor/officeparser/officeParser.cjs`), the V13-4/V13-5 informational closures, the new `packages/core/src/scrub.ts` browser-safe helper file, and the dynamic per-engine tune UI in the netlify demo. **0 new Critical / Important / Minor findings.** Verified zip-slip / XXE / inflate-bomb defenses are preserved byte-identically in the vendored copy. The `createRequire(import.meta.url)` resolution path adds no new attack surface vs. npm-loaded modules.
