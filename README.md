...
| **2,500 – 10,000**    | **FlexSearch**             | Sub-millisecond queries on this size range. Encoded index format is denser than JSON. Loses Fuse's typo tolerance — acceptable for most factual / name searches.                                                          | ✅ **Shipping in 1.3** — `/flexsearch` entry: `createFlexSearchIndex(rows)` + `snippetHTMLForFlexMatch` + `flattenFlexResults`.                           |
| **10,000+ documents** | **Pagefind**               | Chunked on-demand index — only engine here that scales gracefully past five-figure corpora without paying the full-index download cost on first load.                                                                     | ✅ **Shipping in 1.3** — `/pagefind` entry: `emitPagefindHTML(rows, { outDir })` writes one HTML per document for Pagefind's CLI to crawl at build time.  |
...
**v1.3 ships first-party `/flexsearch` and `/pagefind` adapter entries** so consumers crossing the 2,500-doc threshold can swap engines with a one-line import change rather than rewriting the search-UI glue. The live demo currently uses Fuse; a three-engine toggle (Fuse / FlexSearch / Pagefind side-by-side with stats + config samples) is tracked for v1.4 so the visual comparison can get its own focused build pass.
...
### 2026-05-17 — v1.3.0 search-engine-entries audit

A **sixth** adversarial red/blue team pass ran against v1.3.0 (commit on top of shipped 1.2.1). Scope: the two new adapter entries (`/flexsearch` and `/pagefind`), the officeparser promotion to direct dep, the new tsup entries.

**Methodology.** Independent opus-class LLM agent. Source review of `pagefind.ts` (~220 LoC), `flexsearch.ts` (~210 LoC) + new regression tests; targeted re-verification of all 11 prior fixes in their current source positions; 3 probe scripts written to `/tmp/audit6-probes/` and run against the built `dist/` (XSS probes against `emitPagefindHTML`, jail-bypass attempts including symlinks, FlexSearch prototype-pollution and Unicode probes).

**Result: 11 of 11 prior fixes verified, 0 Critical, 0 Important findings.**

**5 new findings on the v1.3 new surface — all addressed before publish:**

| ID    | Severity      | What it was                                                                                                                                                                                                                                                                                                 | What we did                                                                                                                                                                                                                                                                                                                     |
| ----- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| V13-1 | Minor         | `publicDirJail` in `/pagefind` used string-prefix comparison only. If the jail dir contains a symlink that points outside (e.g., `public/escape-link → /tmp/elsewhere`), an `outDir` traversing through it passes the string check but writes outside the jail.                                             | **Fixed in v1.3.0:** added `fs.realpath`-resolved second check after `mkdir`. If the realpath-resolved outDir escapes the realpath-resolved jail, throw with a clear error. Probe script confirmed the symlink bypass is now closed. Pinned by `test/pagefind.test.ts` → `"rejects symlink-based jail escape (V13-1 closure)"`. |
| V13-2 | Minor         | The `baseUrl` option in `emitPagefindHTML` was concatenated into the emitted HTML without escape — a developer-supplied baseUrl containing `<script>` would inject markup into every emitted page's `<a href="...">` and visible link text.                                                                 | **Fixed in v1.3.0:** `baseUrl` now routes through `escapeHTMLText` before concatenation. The API contract is still "developer literal," but the escape closes the only injection path. Pinned by `test/pagefind.test.ts` → `"escapes adversarial baseUrl (V13-2 closure)"`.                                                     |
| V13-3 | Minor         | `(row.format ?? 'pdf').toUpperCase()` was interpolated into `<meta>` and `<span>` without escape. A row passed via JS (bypassing TS types) with `format: { toUpperCase: () => 'PDF"><script>...' }` could inject markup.                                                                                    | **Fixed in v1.3.0:** runtime `typeof === 'string'` guard rejects non-string formats (falls back to `'pdf'`); the resulting string is then HTML-escaped before interpolation. Pinned by `test/pagefind.test.ts` → `"escapes adversarial row.format that returns HTML (V13-3 closure)"`. Test count: 164 → 167.                   |
| V13-4 | Informational | `escapeHTMLText` escapes only the five HTML-meta characters (`& < > " '`). Control bytes (NUL, ESC, BEL, DEL) survive into the emitted HTML body. Not an XSS — browsers render them as zero-width — but ANSI escapes in a downloaded HTML file `cat`'d in a terminal could repaint the operator's terminal. | **Documented, not fixed.** Symmetric with the M3 fix in `extractor.ts` log paths. Tracked for v1.4 if needed; extracted PDF text rarely contains control bytes in practice. No production exposure.                                                                                                                             |
| V13-5 | Informational | `/flexsearch` dynamic-import error message includes raw `e.message` without control-char scrubbing. Realistic exposure is near-zero (Node's module-loader errors are ASCII-clean) but inconsistent with the M3 / I1 pattern elsewhere.                                                                      | **Documented, not fixed.** Same inconsistency exists in `worker.ts:137` (also unscrubbed since 1.2). Worth a sweep in a v1.3.x patch to apply `scrubControl` uniformly across all dynamic-import error paths.                                                                                                                   |

**End-to-end probe results.** 3 probe scripts ran against the built `dist/`:

- `probe1-pagefind-xss.mjs` — `<script>` in row.text/title is HTML-escaped correctly.
- `probe2-pagefind-jail.mjs` — 10 jail-bypass trials; before the V13-1 fix, 1 symlink trial succeeded. After fix: all 10 rejected.
- `probe3-flexsearch.mjs` — 8 adversarial cases including prototype pollution, regex metachars, CJK queries. All HTML-safe.

**Other v1.3 probe results (no finding) worth recording for future audits:**

- HTML-escape of `<script>` in title and text works correctly.
- `<a href="javascript:...">` from `row.url` is preserved verbatim, but URL discovery in `url-scan.ts` only accepts `https?://` / `file://` schemes — `javascript:` URLs can't reach this code path through the documented flow.
- Row id with `pdf-../../etc/passwd` resolves to `outDir/etc/passwd.html` (creates subdirs but doesn't escape outDir). NUL bytes in id are rejected by Node's `writeFile`.
- Duplicate ids silently overwrite (caller's responsibility, not a security issue).
